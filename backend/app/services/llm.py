...
2. GATHER WOOD: If you have no wood (minecraft:oak_log) in inventory_summary, choose MINE_TASK for minecraft:oak_log.
...
class HybridLLMClient:
    ...
    async def get_next_goal(self, observation: Observation) -> PlannerResponse:
        """Sends the observation to the LLM and retrieves the next structured Goal."""
        if settings.is_mock_mode:
            return self._generate_mock_goal(observation)
        
        # Format observation as structured JSON text for the prompt
        obs_json = observation.model_dump_json(indent=2)
        
        try:
            logger.info(f"Sending prompt to LLM ({settings.llm_model})...")
            ...
            logger.error(f"Error communicating with LLM API: {e}. Falling back to rules.")
            return self._generate_mock_goal(observation)
    ...
    def _generate_mock_goal(self, obs: Observation) -> PlannerResponse:
        ...
        # Rule 2: Basic wood gathering loop
        has_logs = any("log" in item.name or "wood" in item.name for item in obs.inventory_summary)
        if not has_logs:
            return PlannerResponse(
                thought="No logs found in inventory. Setting goal to harvest oak logs.",
                goal=Goal(
                    intent="MINE_TASK",
                    target="minecraft:oak_log",
                    count=3,
                    priority="high",
                    reason="Gathering base materials for tools."
                ),
                confidence=0.85
            )
        ...
        if has_logs and not has_planks:
            return PlannerResponse(
                thought="I have logs, now I need to craft wood planks.",
                goal=Goal(
                    intent="CRAFT_TASK",
                    target="minecraft:oak_planks",
                    count=4,
                    priority="normal",
                    reason="Transforming logs to planks."
                ),
                confidence=0.9
            )
